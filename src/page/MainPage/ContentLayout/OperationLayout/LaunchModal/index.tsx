    ...
    return (
        ...
        >
            ...
            >
                <Form.Item label={<></>}>
                    <Radio.Group
                        onChange={radioChangeWrapper(setStatusRadio)}
                        defaultValue={statusRadio}
                    >
                        <Radio value="noStatus">
                            无状态启动<div className="mode-tips">不包含任何初始状态启动</div>
                        </Radio>
                        <Radio value="onStatus">
                            有状态启动<div className="mode-tips">从已存在的有效状态启动</div>
                        </Radio>
                    </Radio.Group>
                </Form.Item>
                ...
                {statusRadio === "noStatus" ? (
                    <Form.Item
                        label={<></>}
                        className="picker-date-form-item"
                    >
                ...
                {statusRadio === "onStatus" ? (
                    <Form.Item
                        name="restoreFrom"
                        label={<></>}
                        className="restore-radio-form-item"
                    >
                        <Radio.Group onChange={radioChangeWrapper(setRestoreRadio)}>
                            <div className="line">
                                <Radio value="last">从最新状态恢复</Radio>
                                {restoreRadio === "last" ? (
                                    <Tooltip title="无法判断该状态和作业的兼容性或存在尚未支持兼容性判断的语法，建议使用其他状态或者无状态启动作业。">
                                        <Tag>
                                            状态兼容性检测：<a style={{ color: "#0064c8" }}>点击检测</a>
                                        </Tag>
                                    </Tooltip>
                                ) : null}
                            </div>
