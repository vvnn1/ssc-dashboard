...
import "./index.sass";
import InputCron from "../../../../../../../component/InputCron";
import { CheckCircleOutlined, DownOutlined } from "../../../../../../../component/Icon";
                    ...
                    <Form>
                        ...
                        {autoCreate ? (
                            <Space.Compact style={{ width: "100%" }}>
                                ...
                                <Dropdown menu={{ items }}>
                                    <Button
                                        style={{ marginLeft: 12 }}
                                        className="quick-cron-setting-btn"
                                    >
                                        快捷设置 <DownOutlined />
                                    </Button>
                                </Dropdown>
                        ...
                        {autoClean ? (
                            <>
                                <Alert
                                    ...
                                    description={
                                        <div className="warn-inner">
                                            <span>
                                                1.快照清理会根据个数或保存时长来一同决定，满足其中一个条件即会被清理。
                                            </span>
                                            <span>
                                                2.快照清理不包含手动点击触发生成的作业快照，此类快照只能在快照查看页面手动删除。
                                            </span>
                                        </div>
                                    }
                                />
                                ...
                                >
                                    <Form.Item>
                                        <Checkbox onChange={checkedChangeWrapper(setSaveCountChecked)}>
                                            历史快照保存个数
                                        </Checkbox>
                                    </Form.Item>

                                    {saveCountChecked ? (
                                        <Form.Item extra="平台会根据此个数定期进行判断，在可以删除对应快照时清理">
                                            <InputNumber style={{ width: 90 }} />
                                        </Form.Item>
                                    ) : null}

                                    <Form.Item>
                                        <Checkbox onChange={checkedChangeWrapper(setSaveTimeChecked)}>
                                            历史快照保存最长时间（单位：小时）
                                        </Checkbox>
                                    </Form.Item>

                                    {saveTimeChecked ? (
                                        <Form.Item extra="平台会根据此时长定期进行判断，在可以删除对应快照时清理">
                                            <InputNumber style={{ width: 90 }} />
                                        </Form.Item>
                                    ) : null}
                                </Form.Item>
